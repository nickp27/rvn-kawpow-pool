...
module.exports = function () {
	...
	Object.keys(poolConfigs).forEach(function (coin) {
		...
		//Functions required for internal payment processing
						...
						if (re.test(workerName)) {
							...
							if (workerName.indexOf(".") !== -1) {
								...
								if (tmp.length !== 2) {
									...
									pool.daemon.cmd("validateaddress", [tmp[0]], function (
										results
									) {
										var isValid =
								...
								//only address
								pool.daemon.cmd("validateaddress", [workerName], function (
									results
								) {
									var isValid =
										results.filter(function (r) {
			...
			handlers.share = function (isValidShare, isValidBlock, data) {
				logger.silly(
					"Handle share, execeuting shareProcessor.handleShare, isValidShare = %s, isValidBlock = %s, data = %s",
					isValidShare,
					isValidBlock,
					JSON.stringify(data)
				);
		...
		pool
			.on("share", function (isValidShare, isValidBlock, data) {
				...
				if (data.blockHash && !isValidBlock) {
					if (workerInfo.length === 2) {
						logger.info(
							"BLOCK>REJECTED> Found block rejected by the daemon, share data: %s" +
								shareDataJsonStr
						);
					} else {
						logger.info(
					...
					if (workerInfo.length === 2) {
						logger.info(
							"BLOCK>ACCEPTED> %s by %s worker: %s",
							data.blockHash,
							workerInfo[0],
							workerInfo[1]
						);
						...
						logger.info(
							"BLOCK>ACCEPTED> %s by %s worker: none",
							data.blockHash,
							workerStr
						);
						logger.info("BLOCK>ACCEPTED>INFO> %s", JSON.stringify(data));
				...
				if (workerInfo.length === 2) {
					if (isValidShare) {
						if (data.shareDiff > 1000000000) {
							logger.warn(
								"SHARE>WARN> Share was found with diff higher than 1.000.000.000!"
							);
						} else if (data.shareDiff > 1000000) {
							logger.warn(
								"SHARE>WARN> Share was found with diff higher than 1.000.000!"
							);
						}
						logger.info(
							"SHARE>ACCEPTED> job: %s req: %s res: %s by %s worker: %s [%s]",
							data.job,
							data.difficulty,
							data.shareDiff,
							workerInfo[0],
							workerInfo[1],
							functions.anonymizeIP(data.ip)
						);
					} else if (!isValidShare) {
						logger.info(
							"SHARE>REJECTED> job: %s diff: %s by %s worker: %s reason: %s [%s]",
							data.job,
							data.difficulty,
							workerInfo[0],
							workerInfo[1],
					...
					if (isValidShare) {
						...
						logger.info(
							"SHARE>ACCEPTED> job: %s req: %s res: %s by %s worker: none [%s]",
							data.job,
							data.difficulty,
							data.shareDiff,
							workerStr,
						...
						logger.info(
							"SHARE>REJECTED> job: %s diff: %s by %s worker: none reason: %s [%s]",
							data.job,
							data.difficulty,
							workerStr,
							data.error,
			...
			.on("difficultyUpdate", function (workerName, diff) {
				...
				if (workerInfo.length === 2) {
					logger.info(
						"DIFFICULTY>UPDATE> diff: %s miner: %s worker: %s",
						diff,
						workerInfo[0],
						workerInfo[1]
					);
					...
					logger.info(
						"DIFFICULTY>UPDATE> diff: %s miner: %s worker: none",
						diff,
						workerStr
					);
				}
	...
	if (portalConfig.switching) {
		let logger = loggerFactory.getLogger(
			`SwitchingSetup[:${parseInt(forkId) + 1}]`,
			"system"
		);
		var proxyState = {};
