...
try {
	...
	try {
		...
		if (cluster.isMaster) {
			logger.warn(
				"POSIX Connection Limit (Safe to ignore) Must be ran as root to increase resource limits"
			);
		}
	} finally {
		...
		if (uid) {
			process.setuid(uid);
			logger.debug(
				"POSIX Connection Limit Raised to 100K concurrent connections, now running as non-root user: %s",
				process.getuid()
			);
		}
	}
...
var buildPoolConfigs = function () {
	...
	poolConfigFiles.forEach(function (poolOptions) {
		...
		if (!(coinProfile.algorithm in algos)) {
			logger.error(
				'[%s] Cannot run a pool for unsupported algorithm "' +
	...
	poolConfigFiles.forEach(function (poolOptions) {
		...
		if (!fs.existsSync(poolFilePath)) {
			logger.warn(
				"Master",
				poolOptions.coinFileName,
				"could not find file: " + poolFilePath
			);
			return;
		}

		var poolProfile = JSON.parse(
			JSON.minify(fs.readFileSync(poolFilePath, {encoding: "utf8"}))
		);
...
var spawnPoolWorkers = function () {
	Object.keys(poolConfigs).forEach(function (coin) {
		...
		if (!Array.isArray(p.daemons) || p.daemons.length < 1) {
			logger.error(
				"[%s] No daemons configured so a pool cannot be started for this coin.",
				coin
			);
	...
	if (Object.keys(poolConfigs).length === 0) {
		logger.warn(
			"PoolSpawner: No pool configs exists or are enabled in pool_configs folder. No pools spawned."
		);
		return;
	}
	...
	var createPoolWorker = function (forkId) {
		...
		worker
			.on("exit", function (code, signal) {
				logger.error(
					"PoolSpawner: Fork %s died, spawning replacement worker...",
					forkId
				);
				setTimeout(function () {
